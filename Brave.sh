...
main() {
  ...
  case "$(whoami)" in
    root) sudo="";;
    *) sudo="$(first_of sudo doas run0 pkexec sudo-rs)" || exit 0;;
  esac
  ...
  if available apt-get && apt_supported; then
    ...
    if ! available curl && ! available wget; then
      $sudo apt-get update >/dev/null 2>&1 || exit 0
    ...
    if pacman -Ss brave-browser >/dev/null 2>&1; then
      ...
      aur_helper="$(first_of paru pikaur yay)" || exit 0
    ...
    available curl || available wget || exit 0
